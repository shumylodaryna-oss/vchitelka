...
const codeFillTasks = [
    {
        desc: "Алгоритм знаходження більшого з двох чисел:",
        code: "ПОЧАТОК\n  Ввести A, B\n  ЯКЩО A > B ТО\n    Вивести ___\n  ІНАКШЕ\n    Вивести B\n  КІНЕЦЬ_ЯКЩО\nКІНЕЦЬ",
        blank: "A",
        options: ["A", "B", "A+B", "A-B"],
        correct: 0
    ...
    {
        desc: "Алгоритм підрахунку суми від 1 до 10:",
        code: "ПОЧАТОК\n  Сума = 0\n  ___ = 1\n  ПОКИ I <= 10\n    Сума = Сума + I\n    I = I + 1\n  КІНЕЦЬ_ПОКИ\n  Вивести Сума\nКІНЕЦЬ",
        blank: "I",
        options: ["Сума", "I", "10", "0"],
        correct: 1
    ...
    {
        desc: "Алгоритм перевірки парності числа:",
        code: "ПОЧАТОК\n  Ввести N\n  ЯКЩО N ___ 2 = 0 ТО\n    Вивести 'Парне'\n  ІНАКШЕ\n    Вивести 'Непарне'\n  КІНЕЦЬ_ЯКЩО\nКІНЕЦЬ",
        blank: "MOD",
        options: ["+", "-", "MOD", "DIV"],
        correct: 2
    ...
    {
        desc: "Алгоритм виведення 'Привіт' п'ять разів:",
        code: "ПОЧАТОК\n  ДЛЯ I = 1 ДО ___\n    Вивести 'Привіт'\n  КІНЕЦЬ_ДЛЯ\nКІНЕЦЬ",
        blank: "5",
        options: ["1", "10", "5", "100"],
        correct: 2
...
let cfStep = 0, cfScore = 0;
function renderCodeFill() {
    if (cfStep >= codeFillTasks.length) {
        document.getElementById('codefill-description').innerHTML = `<span style="color:var(--color-primary)">🎉 Завершено! Результат: ${cfScore} / ${codeFillTasks.length}</span>`;
        document.getElementById('codefill-code').textContent = '';
        document.getElementById('codefill-options').innerHTML = '';
        return;
    }
    const task = codeFillTasks[cfStep];
    document.getElementById('codefill-description').textContent = task.desc;
    document.getElementById('codefill-code').innerHTML = task.code.replace(task.blank, `<span class="codefill-blank" id="cf-blank">???</span>`);
    document.getElementById('codefill-feedback').classList.add('hidden');
    document.getElementById('btn-next-codefill').classList.add('hidden');
    const opts = document.getElementById('codefill-options'); opts.innerHTML = '';
    task.options.forEach((opt, idx) => {
        const btn = document.createElement('button'); btn.classList.add('codefill-option-btn');
        btn.textContent = opt;
        btn.addEventListener('click', () => {
            opts.querySelectorAll('.codefill-option-btn').forEach(b => { b.style.pointerEvents = 'none'; b.classList.remove('selected'); });
            btn.classList.add('selected');
            const blank = document.getElementById('cf-blank');
            blank.textContent = opt;
            const fb = document.getElementById('codefill-feedback');
            fb.classList.remove('hidden');
            if (idx === task.correct) {
                blank.classList.add('filled'); cfScore++;
                fb.className = 'codefill-feedback correct'; fb.textContent = `✅ Правильно! "${opt}" — вірна відповідь.`;
            } else {
                fb.className = 'codefill-feedback wrong'; fb.textContent = `❌ Не вірно. Правильна відповідь: "${task.options[task.correct]}"`;
                document.getElementById('cf-blank').textContent = task.options[task.correct];
            }
            document.getElementById('codefill-score').textContent = `${cfScore} / ${codeFillTasks.length}`;
            document.getElementById('btn-next-codefill').classList.remove('hidden');
        });
        opts.appendChild(btn);
    });
}
document.getElementById('btn-next-codefill').addEventListener('click', () => { cfStep++; renderCodeFill(); });
renderCodeFill();

// =============================================
// ГРА 16: КРОСВОРД
// =============================================
// Simple 7x7 crossword
const crosswordData = {
    grid: [
        [null, null, null, 'B', null, null, null],
        ['A', 'L', 'G', 'O', 'R', 'Y', 'M'],
        [null, null, null, 'T', null, null, null],
        [null, 'M', null, 'S', null, null, null],
        [null, 'O', null, null, null, null, null],
        [null, 'D', 'I', 'S', 'K', null, null],
        [null, 'E', null, null, null, null, null],
        [null, 'M', null, null, null, null, null]
    ],
    numbers: {
        '1-0': 1, '1-3': 2, '0-3': 3, '3-1': 4, '5-1': 5
    },
    clues: {
        across: [
            { num: 1, row: 1, col: 0, len: 7, answer: 'ALGORYM', clue: '1. Послідовність кроків для розв\'язання задачі' },
            { num: 5, row: 5, col: 1, len: 4, answer: 'DISK', clue: '5. Пристрій для зберігання даних' }
        ],
        down: [
            { num: 2, row: 0, col: 3, len: 3, answer: 'BOT', clue: '2. Автоматична програма в мережі' },
            { num: 4, row: 3, col: 1, len: 5, answer: 'MODEM', clue: '4. Пристрій для підключення до інтернету' }
        ]
    }
};

// Simplified crossword with 5 words
const cw = {
    words: [
        { word: 'БАЙТ', row: 0, col: 0, dir: 'across', clue: 'Одиниця інформації = 8 бітів' },
        { word: 'БІТИ', row: 0, col: 0, dir: 'down', clue: 'Найменша одиниця інформації (мн.)' },
        { word: 'АЛГО', row: 2, col: 2, dir: 'across', clue: 'Початок слова "алгоритм"' },
        { word: 'ТЕМА', row: 0, col: 3, dir: 'down', clue: 'Головна думка або предмет' }
    ]
};

// Build a simple manual crossword
const cwDef = {
    size: 8,
    words: [
        { word: 'МОНІТОР', row: 0, col: 0, dir: 'across', num: 1, clue: '1→ Пристрій виведення зображення' },
        { word: 'МЕРЕЖА', row: 0, col: 0, dir: 'down', num: 1, clue: '1↓ Система з\'єднаних комп\'ютерів' },
        { word: 'МИША', row: 2, col: 2, dir: 'across', num: 2, clue: '2→ Пристрій введення, яким клікають' },
        { word: 'ІМПОРТ', row: 0, col: 4, dir: 'down', num: 3, clue: '3↓ Завантаження даних у програму' },
        { word: 'РЯДОК', row: 4, col: 1, dir: 'across', num: 4, clue: '4→ Горизонтальний рядок у таблиці' }
    ]
};

function buildCrossword() {
    const size = cwDef.size;
    ...
    cwDef.words.forEach(w => {
        for (let i = 0; i < w.word.length; i++) {
            const r = w.dir === 'across' ? w.row : w.row + i;
            const c = w.dir === 'across' ? w.col + i : w.col;
            cells[r][c] = w.word[i];
        }
        const key = `${w.row},${w.dir === 'across' ? w.col : w.col}`;
        numbers[`${w.row}-${w.dir === 'across' ? w.col : w.col}`] = w.num;
    });
    ...
    for (let r = 0; r < size; r++) {
        for (let c = 0; c < size; c++) {
            ...
            if (cells[r][c]) {
                ...
                if (num) { const n = document.createElement('div'); n.classList.add('cell-number'); n.textContent = num; cell.appendChild(n); }
                const inp = document.createElement('input');
                inp.setAttribute('maxlength', '1');
                inp.setAttribute('data-answer', cells[r][c]);
    ...
    cwDef.words.forEach(w => {
        const div = document.createElement('div'); div.classList.add('clue-item');
        div.textContent = w.clue;
        if (w.dir === 'across') acrossEl.appendChild(div);
...
document.getElementById('btn-check-crossword').addEventListener('click', () => {
    ...
    inputs.forEach(inp => {
        ...
        if (inp.value.toUpperCase() === inp.dataset.answer) { inp.classList.add('correct-cell'); correct++; }
        else { inp.classList.add('wrong-cell'); }
    });
    const result = document.getElementById('crossword-result');
    result.classList.remove('hidden');
    if (correct === total) { result.className = 'crossword-result correct'; result.textContent = `🎉 Чудово! Всі ${total} літер правильні!`; }
    else { result.className = 'crossword-result partial'; result.textContent = `${correct} / ${total} літер правильних. Спробуй ще!`; }
});
document.getElementById('btn-reset-crossword').addEventListener('click', () => {
    document.querySelectorAll('#crossword-grid input').forEach(i => { i.value = ''; i.classList.remove('correct-cell','wrong-cell'); });
    document.getElementById('crossword-result').classList.add('hidden');
});
buildCrossword();
